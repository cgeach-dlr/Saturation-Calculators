...
nu_Ls = (np.arange(25) - 11)*210e6 #Hz
lambda_Ls = nu_Ls/nu0 * lamb0 #m

ca_sats_vdG_nu_Ls = np.zeros(len(nu_Ls))
ca_sats_vdG_nu_Ls2 = np.zeros(len(nu_Ls))
ca_sats_vdG_nu_Ls3 = np.zeros(len(nu_Ls))
ca_plus_sats_vdG_nu_Ls = np.zeros(len(nu_Ls))
ca_plus_sats_vdG_nu_Ls2 = np.zeros(len(nu_Ls))
ca_plus_sats_vdG_nu_Ls3 = np.zeros(len(nu_Ls))
...
for i in range(len(nu_Ls)):
    nu_L = nu_Ls[i]    
    ca_sats_vdG_nu_Ls[i] = ca_lib.get_saturation_beam(nu_L, ca_Delta_nu_L,
                ca_N_L, z, T_atm, alpha_L, alpha_T, ca_t_L, nt, delta_t,
                delta_r, Temp_Ca, 'lorentzian', ratio_beam=True)
    ca_sats_vdG_nu_Ls2[i] = ca_lib.get_saturation_beam(nu_L, ca_Delta_nu_L/10,
                ca_N_L, z, T_atm, alpha_L, alpha_T, 10*ca_t_L, nt, 10*delta_t,
                delta_r, Temp_Ca, 'lorentzian', ratio_beam=True)
    ca_sats_vdG_nu_Ls3[i] = ca_lib.get_saturation_beam(nu_L, ca_Delta_nu_L/10,
                ca_N_L, z, T_atm, alpha_L2, alpha_T, 10*ca_t_L, nt, 10*delta_t,
                delta_r, Temp_Ca, 'lorentzian', ratio_beam=True)    
    ca_plus_sats_vdG_nu_Ls[i] = ca_plus_lib.get_saturation_beam(nu_L, 
                ca_plus_Delta_nu_L, ca_plus_N_L, z, T_atm, alpha_L, alpha_T,
                ca_plus_t_L, nt, delta_t, delta_r, Temp_Ca, 'lorentzian',
                ratio_beam=True)
    ca_plus_sats_vdG_nu_Ls2[i] = ca_plus_lib.get_saturation_beam(nu_L,
                ca_plus_Delta_nu_L/10, ca_plus_N_L, z, T_atm, alpha_L, alpha_T,
                10*ca_plus_t_L, nt, 10*delta_t, delta_r, Temp_Ca, 'lorentzian',
                ratio_beam=True)
    ca_plus_sats_vdG_nu_Ls3[i] = ca_plus_lib.get_saturation_beam(nu_L,
...
Spectrum_data = np.vstack((lambda_Ls*1e12, 100*ca_sats_vdG_nu_Ls,
                   100*ca_sats_vdG_nu_Ls2, 100*ca_sats_vdG_nu_Ls3,
                   100*ca_plus_sats_vdG_nu_Ls, 100*ca_plus_sats_vdG_nu_Ls2,
                   100*ca_plus_sats_vdG_nu_Ls3))

np.savetxt(os.path.join(outpath_data, 'Ca_and_ca_plus_spectrum.txt'),
           Spectrum_data.T, delimiter=',')

# Generate plot for Figure 1
plt.figure(figsize=(12,8))

plt.plot(lambda_Ls*1e12, 100*ca_sats_vdG_nu_Ls, label='Ca')
plt.plot(lambda_Ls*1e12, 100*ca_plus_sats_vdG_nu_Ls, label='Ca+')
plt.legend()
...
plt.savefig(os.path.join(outpath_figs, 'Ca_and_ca_plus1.jpg'), dpi=300)

# Generate plot for Figure 3
plt.figure(figsize=(12,8))

plt.plot(lambda_Ls*1e12, 100*ca_sats_vdG_nu_Ls2, c='tab:blue', label='Ca')
plt.plot(lambda_Ls*1e12, 100*ca_sats_vdG_nu_Ls3, c='tab:blue', linestyle=':')
plt.plot(lambda_Ls*1e12, 100*ca_plus_sats_vdG_nu_Ls2, c='tab:orange',
         label='Ca+')
plt.plot(lambda_Ls*1e12, 100*ca_plus_sats_vdG_nu_Ls3, c='tab:orange',
         linestyle=':')
plt.legend()
plt.ylabel('Saturation percent')
plt.xlabel('Central laser wavelength (pm)')
...
plt.savefig(os.path.join(outpath_figs, 'Ca_and_ca_plus3.jpg'), dpi=300)
...
nu_Ls_errs = (np.arange(6)-2) * 400e6

for i in range(len(Es)):
    ca_N_L = ca_lib.N_L_from_pulse_energy(Es[i])
    ca_Res = ca_lib.get_wind_and_temp_errors(Temp_Ca, nu_Ls_errs,
                                             ca_Delta_nu_L, ca_N_L, z, T_atm,
                                             alpha_L, alpha_T, ca_t_L, nts[i],
                                             100/nts[i], delta_r, 'lorentzian')
    ca_Res2 = ca_lib.get_wind_and_temp_errors(Temp_Ca, nu_Ls_errs,
                                              ca_Delta_nu_L/10, ca_N_L, z,
                                              T_atm, alpha_L, alpha_T,
                                              10*ca_t_L, nts[i], 1000/nts[i],
                                              delta_r, 'lorentzian')
    ca_Res3 = ca_lib.get_wind_and_temp_errors(Temp_Ca, nu_Ls_errs,
                                              ca_Delta_nu_L/10, ca_N_L, z,
    ...
    ca_plus_Res = ca_plus_lib.get_wind_and_temp_errors(Temp_Ca, nu_Ls_errs,
                                                       ca_plus_Delta_nu_L,#
                                                       ca_plus_N_L, z, T_atm,
                                                       alpha_L, alpha_T,
                                                       ca_plus_t_L, nts[i],
                                                       100/nts[i], delta_r,
                                                       'lorentzian')
    ca_plus_Res2 = ca_plus_lib.get_wind_and_temp_errors(Temp_Ca, nu_Ls_errs,
                                                        ...
                                                        1000/nts[i], delta_r,
                                                        'lorentzian')
    ca_plus_Res3 = ca_plus_lib.get_wind_and_temp_errors(Temp_Ca, nu_Ls_errs,
                                                        ca_plus_Delta_nu_L/10,
                                                        ca_plus_N_L, z, T_atm,
                                                        alpha_L2, alpha_T,
                                                        10*ca_plus_t_L, nts[i],
...
Temp_bias_data = np.vstack((Es, ca_T_err_200, ca_T_err_200_2, ca_T_err_200_3,
                            ...
                            ca_plus_T_err_200_3))
np.savetxt(os.path.join(outpath_data, 'Ca_and_ca_plus_temp_biases.txt'),
...
plt.savefig(os.path.join(outpath_figs, 'Ca_and_ca_plus2.jpg'), dpi=300)
