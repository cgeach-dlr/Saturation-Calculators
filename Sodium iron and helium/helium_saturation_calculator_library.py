...
M_He = 0.004003 #kg mol^-1
m_He = M_He/N_A

lamb0 =  1083.3306444 * 10**-9 #m, in vacuum
nu0 = c_light / lamb0
A_i = 10.22e6 #Hz
tau_R = 1 / A_i * 1e9 #ns
Delta_nu_n = A_i / (2 * np.pi) #Hz
absorb_coeff = e_elec**2/(4*eps_0 * m_elec * c_light)
sig_D = nu0*np.sqrt(k_B / (m_He * c_light**2))
f_D = 0.539
...
nu_11 = 2.29116 * 10**9 #Hz
nu_12 = 0 #Hz

#Only transitions 1_1 and 1_2 are considered.
g_jk = np.array([g1_1, g1_2])
nu_jk = np.array([nu_11, nu_12])

#Hanle factors - assumed to be 1 here.
q_jk = np.array([1, 1]) 

delta_nu = 3e5
nu_shifts = np.arange(-12*10**9, 12*10**9, delta_nu)
nv = len(nu_shifts)
...
def get_natural_absorption_line(k):
   ...
   # to the kth excited state (natural linewidth only)
    ...
    natural_absorption_line = (g_jk[k] / sum_g * absorb_coeff * f_D / np.pi * 
                       Delta_nu_n / 2 / ((nu_shifts - nu_jk[k])**2
                                         + (Delta_nu_n/2)**2))
    return natural_absorption_line

def get_combined_absorption_line():
    #Returns the combined absorption line for both 2^3S <--> 2^3P transitions.
    
    combined_line = np.zeros(len(nu_shifts))
    for k in range(2):
        combined_line += get_natural_absorption_line(k)
            
    return combined_line

def get_temperature_spectrum(Temp_He): 
...
def get_effective_absorption_lines(nu_L=0, Delta_nu_L = 100*10**6,
    ...
    for k in range(2):
        alpha_jk = absorb_coeff * g_jk[k] / sum_g * (f_D / np.pi * Delta_nu_n
                   / 2 / ((nu_shifts + nu_jk[k])**2 + (Delta_nu_n/2)**2))
        L_jk[k] = convolve(laser_spectrum, alpha_jk)
    return L_jk

def get_total_scattering_cross_section_spectrum(Temp_He, Delta_nu_L=100e6,
                                                lineshape='gauss'):
...
def get_total_scattering_cross_section(Temp_He, nu_L=0, Delta_nu_L=100e6,
                                       lineshape='gauss'):
    #Returns the total effective Doppler-broadened scattering cross-section
    # for a given laser frequency
    
    sigma_tot = get_total_scattering_cross_section_spectrum(
        Temp_He, Delta_nu_L, lineshape)
...
def fit_wind_and_temp(params, nu_Ls, ydata, Delta_nu_L=100e6,
    ...
    fit = params[0]*get_total_scattering_cross_section(
        params[1], nu_Ls - params[2]/lamb0, Delta_nu_L, lineshape)
    return fit - ydata
...
def get_saturation(nu_L, Delta_nu_L, N_L, z, alpha_L, T_atm, t_L=10, nt=50,
    ...
    try:
        ...
        print('No abridgement possible. Warning:' +  
              'only {0:.2f} percent of photons accounted for.'.format(
                  np.sum(N)/N_L * 100))    
    n = np.zeros((nv, nt))
    n_e = np.zeros((2, nv, nt))
    
    n[:,0] = np.ones(nv)
    
    n_e2 = np.zeros((2, nv, nt))
    ...
    for i in range(nt - 1):
        ...
        n[:,i+1] = n[:,i] + ((n_e[0,:,i] + n_e[1,:,i]) / tau_R * delta_t
                                - (n[:,i] - n_e[0,:,i]) * f_jk[0,:] 
                                - (n[:,i] - 3 / 5 * n_e[1,:,i]) * f_jk[1,:])
        n_e[0,:,i+1] = n_e[0,:,i] + (-n_e[0,:,i] / tau_R * delta_t
                                    + (n[:,i] - n_e[0,:,i]) * f_jk[0,:]) 
        n_e[1,:,i+1] = n_e[1,:,i] + (-n_e[1,:,i] / tau_R * delta_t
                                  + (n[:,i] - 3 / 5 * n_e[1,:,i]) * f_jk[1,:]) 

        P_s += (np.sum((q_jk[0] * n_e[0,:,i] + q_jk[1] * n_e[1,:,i])
                      * temp_spectrum) / tau_R * delta_t
        ...
        P_ns += (np.sum((q_jk[0] * n_e2[0,:,i] + q_jk[1] * n_e2[1,:,i]) 
                        * temp_spectrum) / tau_R * delta_t
                        / np.sum(temp_spectrum))
    
    P_s += (np.sum((q_jk[0] * n_e[0,:,i+1] + q_jk[1] * n_e[1,:,i+1])
                    * temp_spectrum) / np.sum(temp_spectrum))
    P_ns += (np.sum((q_jk[0] * n_e2[0,:,i+1] + q_jk[1] * n_e2[1,:,i+1])
                     * temp_spectrum) / np.sum(temp_spectrum))
...
def get_saturation_beam(nu_L, Delta_nu_L, N_L, z, T_atm, alpha_L, alpha_T,
                        ...
                        Temp_He=1000, lineshape='gauss', ratio_beam=True):
    #Returns the expected degree of saturation, according to the VDG approach,
...
def get_wind_and_temp_errors(Temp_He, nu_Ls, Delta_nu_L, N_L, z, T_atm,
    ...
    norm = get_total_scattering_cross_section(Temp_He, 0)
    
    for i in range(len(nu_Ls)):
        nu_L = nu_Ls[i]
        Ps[:,i] = get_saturation_beam(nu_L, Delta_nu_L, N_L, z, T_atm,
                                      alpha_L, alpha_T, t_L, nt, delta_t,
                                      delta_r, Temp_He, lineshape, False)
    
    p = np.array([np.max(Ps)/norm, Temp_He, 1])
