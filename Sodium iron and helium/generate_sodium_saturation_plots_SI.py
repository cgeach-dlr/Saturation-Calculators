...
for i in range(len(Es)):
    ...
    Res = na_lib.get_wind_and_temp_errors(Temp_Na, nu_Ls_errs, Delta_nu_L,
                                          N_L, z, T_atm, alpha_L, alpha_T, t_L,
                                          nt, delta_t, delta_r, lineshape)
...
np.savetxt(os.path.join(outpath, 'Na_saturation_SI.txt'), Na_spectrum_data.T,
           delimiter=',')

Na_temp_and_wind_biases_data = np.vstack((u_0s, dens_err, T_err, w_err))
np.savetxt(os.path.join(outpath, 'Na_temp_and_wind_biases_SI.txt'),
